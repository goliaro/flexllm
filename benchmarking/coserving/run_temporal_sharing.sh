...
# TEMPORAL_SHARING_FREQUENCIES=(256 512 1024)
TEMPORAL_SHARING_FREQUENCIES=(64)
for temporal_sharing_frequency in "${TEMPORAL_SHARING_FREQUENCIES[@]}"; do
  OUTPUT_FOLDER="../../benchmarking/output/e2e/temporal_sharing/${temporal_sharing_frequency}"
  # mkdir -p "$OUTPUT_FOLDER/output" "$OUTPUT_FOLDER/logs" "$OUTPUT_FOLDER/profiling"

  # Note: All code below (including the loop over MODEL_NAMES and QPS, etc.)
  # should be indented inside this loop. Also, in the peft_train call add:
  ...
  for i in "${!MODEL_NAMES[@]}"; do
      ...
      TRACES_FOLDER_="${TRACES_FOLDER}/${MODEL_TYPE}"
      ...
      for qps in "${QPS_vals[@]}"; do
          ...
          LOG_FILE="${OUTPUT_FOLDER}/logs/${MODEL_NAME//\//_}_${trace}_bz_${BATCH_SIZE}_tokens_per_batch_${MAX_TOKENS_PER_BATCH}_kv_cache_slots_${NUM_KV_CACHE_SLOTS}_${qps}_qps_${PEFT_SUPPORT_MODE}.log"
          rm $OUTPUT_FILE $LOG_FILE || true
          
          echo "Running $MODEL_NAME (tp=$NGPUS) on $trace with BZ=$BATCH_SIZE, TOKENS_PER_BATCH=$MAX_TOKENS_PER_BATCH, KV_CACHE_SLOTS=$NUM_KV_CACHE_SLOTS, NUM_BWD_LAYERS=$NUM_BWD_LAYERS, QPS=$qps, PEFT_SUPPORT_MODE=$PEFT_SUPPORT_MODE, TEMPORAL_SHARING_FREQUENCY=$temporal_sharing_frequency"
